...
let Vendors_Choice1="";
let Price_StartWith1= "";
var objects_storage = {
    Vendors_Choice : null,
    Price_StartWith : null
};

describe('Alation-SDET Assignment',function(){
    beforeEach(function () {
        isAngularSite(false);
    ...
    afterAll(function(){
        ...
        describe('Amazon.com is used for SDET Assignment - General TestCases',function(){
            ...
            it("TestCase 3 : Read the Book Title",function () {
               ...
               // element(by.id("productTitle")).getText().then(function (getText) {
                    console.log("Selected Book Name is "+ getText);
                });
            });
            it("TestCase 4 : Book written by Authors. List their Names",function() {
            ...
            it("TestCase 5 : Read the Price RANGES (PaperBack,Kindle) of NewBook", function () {
             ...
             /* element(by.xpath('//li[@id="mediaTab_heading_1"]/a/span/div[2]/span')).getText().then(function(PaperBackPrice){
                    console.log("paperBack Price is "+ PaperBackPrice)
                });
                element(by.xpath('//li[@id="mediaTab_heading_0"]/a/span/div[2]/span')).getText().then(function(KindlePrice){
                    console.log("Kindle Price is "+ KindlePrice)
                });
                */
            });
            it("TestCase 6 : Read the Book ISBN-13 and ISBN-10 Bar Code Values",function () {
                ...
                //expect(element(by.xpath(".//*[@id='isbn_feature_div']")).isPresent()).toBe(true);
        ...
        describe(" TestCases Related to 'PAPER BACK' Book",function(){
            ...
            it("TestCase 6 : Buy New Book Price of PaperBack",function(){
               ...
               // locators.getTextby_ClassName("a-size-medium a-color-price header-price").then(function (getText_Price) {
            ...
            it("TestCase 7 : Verify 'ADD TO CART' navigates to AddedToCart_Page",function () {
                locators_Objects.clickby_Xpath("//input[@id='add-to-cart-button']");
                expect(browser.getTitle()).toContain('Amazon.com Shopping Cart');
                browser.navigate().back();
            });

            it("TestCase 7 : Verify New Book is 'IN STOCK' or Not ",function(){
                //var Output=element(by.className("a-size-medium a-color-success")).getText().then(function (result) {
            ...
            it("TestCase 8 : Product Delivery COuntry and expected date",function(){
                locators_Objects.getTextby_XPath('//*[@id="delivery-message"][1]/b[1]').then(function(country){
              //  element(by.xpath('//*[@id="delivery-message"][1]/b[1]')).getText().then(function(country){
                    console.log("Delivery COuntry is : "+ country);   
                });
                locators_Objects.getTextby_XPath('//*[@id="delivery-message"][1]/b[2]').then(function(Date){
               // element(by.xpath('//*[@id="delivery-message"][1]/b[2]')).getText().then(function(Date){
                    console.log("Delivery Date Expected is : "+ Date);   
                });

            });
            it("TestCase 9 : Verify 'Add to Card','Buy Now','Quantity' and 'See All Buying Options' is Present and Enabled for Selection",function(){
            ...
            it("TestCase 10 : Read More Buying Options vendors count and  Price Range",function () {
                browser.sleep(5000);
                var MoreBuyingOptions= locators_Objects.verify_Is_Present_by_Xpath(".//*[contains(text(),'More Buying Choices')]",true);
                //expect(element(by.xpath(".//*[contains(text(),'More Buying Choices')]")).isPresent()).toBeTruthy();
               // var MoreBuyingOptions = element(by.xpath(".//*[contains(text(),'More Buying Choices')]")).isPresent();
                if(MoreBuyingOptions){
                    ...
                    //  Vendors_Choice1 = element(by.xpath(".//*[@id='mediaOlp']//div[2]/div/span[1]/a")).getText().then(function (vendors) {
                         return vendors;
                    });
            ...
            it("TestCase 11 : Read the Price of USED BOOK",function () {
                var BuyUsed = locators_Objects.verify_Is_Present_by_Xpath(".//*[contains(text(),'Buy used')]",true);
                //expect(element(by.xpath(".//*[contains(text(),'Buy used')]")).isPresent()).toBeTruthy();
                if(BuyUsed){
            ...
            it("TestCase 12 : Buy Used - Verify 'Buy Now' Option is available for Used Book and 'Add to Cart' option is available ",function () {

                locators_Objects.verify_Is_Present_by_Xpath(".//*[@id='usedOfferAccordionRow']//*[contains(text(),'Buy Now')]",false);
               // expect(element(by.xpath(".//*[@id='usedOfferAccordionRow']//*[contains(text(),'Buy Now')]")).isPresent()).toBe(false);
                browser.driver.wait(EC.elementToBeClickable(element(by.name('submit.add-to-cart-ubb'))), TimeInSec,FailureMessage);
               // locators_Objects.verifyElement_Is_Clickable_Xpath(".//*[@id='addToCart']//*[@id='add-to-cart-button-ubb']",5000,"'Add to Cart is not Enabled for selection'");

            });
            it("TestCase 13 : Verify Used Book is 'IN STOCK' or Not ",function(){
                var InstockIsAvailable = locators_Objects.verify_Is_Present_by_Xpath("//*[@id='usedOfferAccordionRow']//*[contains(text(),'In Stock')]",true);
                //expect(element(by.xpath("//*[@id='usedOfferAccordionRow']//*[contains(text(),'In Stock')]")).isPresent()).toBeTruthy();
